...
r = requests.get('https://spreadsheets.google.com/feeds/list/1GqKy779qrXnnMBpmt3QoBJPvJirKWewX99ImQ2xqg70/od6/public/values?alt=json')
data = r.json()
data_list = data["feed"]["entry"]
...
def chart():
    ...
    it = [10, 9, 8, 7, 6, 4, 7, 8]
    c = ["#F7464A", "#46BFBD", "#FDB45C", "#FEDCBA", "#ABCDEF", "#DDDDDD", "#ABCABC"]
    colors = ["#F7464A", "#46BFBD", "#FDB45C", "#FEDCBA", "#ABCDEF", "#DDDDDD", "#ABCABC","#FDB45C", "#FEDCBA", "#ABCDEF", "#DDDDDD", "#ABCABC"]
    names=[]
    values=[]
    labels = ["promedio",
                   "respuestas",
    ...
    for i in data_list:
        names.append(i["title"]["$t"])
        values.append( [float(i["gsx$promedio"][ "$t"]),
                        float(i["gsx$respuestas"]["$t"]),
                        float(i["gsx$creditos"]["$t"]),
                        float(i["gsx$creditos_2"]["$t"]),
                        float(i["gsx$creditos_3"]["$t"]),
                        float(i["gsx$creditos_4"]["$t"]),
                        float(i["gsx$creditos_5"]["$t"]),
                        float(i["gsx$estudio"]["$t"]),
                        float(i["gsx$tareas"]["$t"]),
                        float(i["gsx$proyectos"]["$t"]),
...
#     return render_template('try.html', set=zip(values, labels, colors))
